...
PARAMETER(a=1.6,pi=3.14159,mpbar=10.2,&							! mpbar=conversion factor from MPa to bar
					g=9.81,rho=1000.,degree=0.0174533)			
...
REAL(double), ALLOCATABLE, SAVE :: avparvec(:),&
...
REAL(double), SAVE :: dt,cz,cgs,lat								! lat=geogr. latitude
...
!*--------------------------------------------------------------------
!			
		parvec=2.0804*srad										! (Out[17]), par in mol/m2 if srad was MJ/m2
		do in=1,N
			daylength=12. - 7.639437*ASin((0.397949*Cos(0.172142 + &				! (Out[22]), in hours
			...
			do ik=1,24											! Loop through every hour of day, where ik=hour
				ii=in*24+ik-24
				tair=tamean + dtr*(0.0138*Cos(3.513 - ((-1 + ik)*Pi)/&				! (Out[38], accounts for diurnal variation in air temperature
					3.) + 0.0168*Cos(0.822 - ((-1 + ik)*Pi)/4.) + &					! (derived from 3.52+3.53) 
					0.0984*Cos(0.36 - ((-1 + ik)*Pi)/6.) + &
					0.4632*Cos(3.805 - ((-1 + ik)*Pi)/12.))
				tairh(ii)=tair
				vd=0.006028127*2.718282**((17.27*tair)/(237.3 +&					! (Out[52]), accounts for diurnal variation in vapour deficit
					tair)) - 9.869233e-6*vp											! (derived from 3.54+3.55) 
				if(vd.le.0.)then
					vd=0.0
				endif
				gammastarvec(ii)=0.00004275*E ** ((18915.*(-25. + tair))/&			! (Out[274], derived from (3.25))
					(149.*R*(273. + tair)))
				vdh(ii)=vd
				rainh(ii)=rainvec(in)/(24.0*3600.0*1000.0)							! average rainfall in hour ii (m/s)
				if (sunr.le.ik .and. ik+1.le.suns) then
					parh(ii)=(-0.000873*parvec(in)*Cos(0.017453*lat)*&				! ([Out30]), in mol/m2/s
						Sqrt(0.920818 - 0.079182*Cos(0.034428*&						! (derived from 3.51) accounts for diurnal variation in global irradiance
				...
				gammastarvec(ii)=0.00004275*E ** ((18915.*(-25. + tairh(ii)))/&			! (Out[274], derived from (3.25))
					(149.*R*(273. + tairh(ii))))
			enddo
		enddo
		close(102)
	...
	do i=1,Nh
		...
		oldh=h
		gammastarvec(i)=0.00004275*E ** ((18915.*(-25. + tairh(i)))/&				! (Out[274], derived from (3.25)) 
			 (149.*R*(273. + tairh(i))))
	enddo
	close(102)
	parsaved=1	
...
sunewvec(nlayersnew)=(1 + (alphavg*(depth - ysnew))**nvg)**(-mvg)	! (Out[58])
do i=nlayersnew-1,1,-1											
	depth=depth+(delyunewvec(i)+delyunewvec(i+1))/2
	sunewvec(i)=(1 + (alphavg*(depth - ysnew))**nvg)**(-mvg)		! (Out[58])
enddo
...
suvec=sunewvec
wsvec=wsnewvec
pcapvec(1:nlayersnew)= (-1 + sunewvec(1:nlayersnew)**(-1/mvg))**&	! (Out[54])
		(1/nvg)/alphavg
...
!*--------------------------------------------------------------------
!
tc=tcf*pc*2.5													! (3.38) 	foliage tunrover costs, assuming crown LAI of 2.5
!*--------------------------------------------------------------------
!*    Daily loops
!*--------------------------------------------------------------------
!
...
902	do while (d.lt.dtest)
		...
		jmax25=Max(jmax25,50.0e-6)								! making sure that the values don't become too low, otherwise they could never pick up again
		jmax25g= jmax25g(2)*(/0.99,1.0,1.01/)
		jmax25g=Max(jmax25g,50.0e-6)
		pcg= pcg(2)+(/-0.02,0.0,0.02/)							! vector with values varying by 1%
		pcg= Max(pcg,0.)
		pcg(3)=Min(Max(pcgmin,pcg(3)),1.0-pc)
		tcg=tcf*pcg*2.5											! (3.38) foliage turnover costs, assuming LAI/pc of 2.5
		rr=2.55e-7*Sum(rsurfvec(1:pos))							! (3.40), (Out[190])		root respiration [mol/s]
		if(pos.gt.nlayersnew) then
			cpcc=cpccf*pc*rootdepth+mdstore*2.45e-10			! (3.42, 2.45e-10 from (Out[165])) costs of water distribution and storage		
		else													
		...
		rrg=2.55e-7*Sum(rsurfg(1:posg))							! (3.40), (Out[190])		root respiration grasses [mol/s] 
...
!*--------------------------------------------------------------------	
		do h=1,24												! loops through each hour of daily dataset
			...
			vd=vdh(ii)
			par=parh(ii)
			jmax=(E**((ha*(-25. + tair)*(-273. + topt + 273.*r*topt))/&			! (Out[310], derived from (3.26)) Temperature dependence of Jmax
			...
			jmaxg=(E**((ha*(-25. + tair)*(-273. + topt + 273.*r*topt))/&		! (Out[310], derived from (3.26)) Temperature dependence of Jmax
				((25. + 273.*r*topt)*(tair + 273.*r*topt)))*&
				((-1. + E**(-(hd*(-298. + topt))/(25. + 273.*r*topt)))*ha +& 
				hd)*jmax25g)/&
				((-1. + E**((hd*(273. + tair - topt))/(tair + 273.*r*topt)))*&
			...
			if (optmode.eq.0) then												! daily recalculation for resultsdaily
				rainvec(d)=rainvec(d)+rain*3600.*1000.							! mm/d 
				parvec(d)=parvec(d)+par*3600.									! in mol/m2/d
				srad(d)=parvec(d)/2.0804										! MJ/m2/d
...
!
			if (par.gt.0.0)then
				jact=(1. - E**(-(alpha*par)/jmax))*jmax*pc						! (3.23), (Out[311])
				jactg(1,:)=(1. - E**(-(alpha*par)/jmaxg))*jmaxg*pcg(1)			! (3.23), (Out[311]) 
				jactg(2,:)=(1. - E**(-(alpha*par)/jmaxg))*jmaxg*pcg(2)			! (3.23), (Out[311])
				jactg(3,:)=(1. - E**(-(alpha*par)/jmaxg))*jmaxg*pcg(3)			! (3.23), (Out[311])
				if (vd.gt.0.0.and.lambda.gt.(2.*a*vd)/(ca + 2.*gammastar)&
						.and.jact(2).gt.(4*ca*rl(2) + 8*gammastar*rl(2))/&
						(ca - gammastar)) then
					gstom=Max(0.,(0.25*(a*(ca*(jact(2) - 4.*rl(2)) - 4.*&		! (Out[314]) 
				...
				where (vd.gt.0.0.and.lambdag.gt.(2.*a*vd)/(ca + 2.*gammastar)&
					...
					gstomg=Max(0.,(0.25*(a*(ca*(jactg-4.*rlg)-4.*&				! (Out[314]) 
...
!
				pcapvec(1:nlayers)= (-1 + suvec(1:nlayers)**(-1/mvg))**&		! (Out[54]), Chapter 3.3.2.3 
						(1/nvg)/alphavg
				...
				rsoilvec(1:postemp)=(Sqrt(Pi/2.)*Sqrt((rootrad*omgu*&			! soil resistance, (Out[ 241] with svolume=omgu*delyuvec(1:postemp)); derived from (3.32)
				...
				if(Maxval(etmg).gt.0.) then
					...
					if(Sum(ruptkg).gt.0.) then
						where(etmg.gt.Sum(ruptkg)) 
							rootlim=1.0
							etmg=Sum(ruptkg)
							transpg=etmg*55555.555555555555						! (Out[249])			mol/s=m/s*10^6 g/m/(18g/mol)
							gstomg=transpg/(a*vd)
						endwhere
				...
				if (rain.gt.0.) then
					inf=Min((ksat+kunsatvec(1))/2.*omgu*(1+(2.*&				! (3.6), (Out[60]) 
				...
				do i=1,nlayers-1
					qblvec(i)= -(omgu*(1. + (-pcapvec(i) + pcapvec(i+1))/&		! (3.4), (Out[62]) 
							(0.5*delyuvec(i) + 0.5*delyuvec(i+1)))*&
							0.5*(kunsatvec(i) + kunsatvec(i+1)))
				enddo
				qblvec(nlayers)= -(omgu*(1. + (-pcapvec(nlayers)/&				! (3.5), (Out[62]) 
							(0.5*delyuvec(nlayers))))*0.5*&
							(kunsatvec(nlayers)+ksat))
				esoil= 0.0002*(1.-0.8*(pc+pcg(2)))*par*suvec(1)*omgu				! (3.9), (Out[141])
				esoils= 0.0002*(1.-0.8*(pc+pcg(2)))*par*omgo						! (3.10), (Out[142], but suvec(1) instead of par removed)
				spgfcf= (0.5*ksat*omgo*(ys - zr))/(cgs*Cos(go))					! (3.7), (Out[61])
903				dys=(esoils + spgfcf + qblvec(nlayers))/(epsln*(-1. +&			! (3.15), (Out[144], with suavg=Sum(suvec(....)/nlayers) 
				...
				if(ys.gt.zr) then												
					domgu= -dys/(2.*Sqrt((cz - ys)*(cz - zr)))					! (Out[146])
					dyu= (dys*(-cz + zr))/(2.*Sqrt((cz - ys)*(cz - zr)))		! (Out[148])
					if(dys.le.0.) then
...
!
				mqss= Max(0.9*mqx,(mqx*(mpbar*(md*md+752.*md*mqx+mqx*mqx)*&		! (Out[257]) steady-state Mq
				...
				if (mq.le.0.9*mqx) then											! makes sure that tissue water does not get below 0.9mqx
					if (etm.gt.0.9*Sum(ruptkvec(1:nlayers))) then
						if (Sum(ruptkvec(1:nlayers)).ge.0.) then
							...
							transp=etm*55555.555555555555						! (Out[249])			mol/s=m/s*10^6 g/m/(18g/mol)
							gstom=transp/(a*vd)
						else
						...
						mqss= Max(0.9*mqx,(mqx*(mpbar*(md*md+752.*md*mqx+&		! (Out[257]) steady-state Mq
...
901				if(Abs(mq-mqss).gt.mqx/1.e6) then
				...
				if(dt.eq.dtss) then			
					pcapnewvec(1:nlayers)= (-1 + (suvec(1:nlayers)+&			! (Out[54]) 
						dsuvec(1:nlayers)*dt)**(-1/mvg))**(1/nvg)/alphavg
					pcapnewvec(nlayers+1:M)=0.
					mqssnew=Max(0.9*mqx,(mqx*(mpbar*(md*md+752.*md*mqx+&		! (Out[257]) steady-state Mq
						mqx*mqx)*Sum((rsurfvec(1:postemp)/&
						(rrootm + rsoilvec(1:postemp)))) - (md + mqx)*&
						(md+mqx)*(etm-Sum(((-phydrostaticvec(1:postemp)&
						-pcapvec(1:postemp))*rsurfvec(1:postemp))/&
...
!
				ass= (4.*ca*gstom + 8.*gammastar*gstom + jact - 4.*rl - &		! (3.22); (Out[319])
					Sqrt((-4.*ca*gstom + 8.*gammastar*gstom + jact - &
					4.*rl)**2. + &
					16.*gammastar*gstom*(8.*ca*gstom + jact + 8.*rl)))/8.
				hass=hass+ass*dt
				assg= (4.*ca*gstomg + 8.*gammastar*gstomg+jactg - 4.*rlg-&		! (3.22); (Out[319])
...
!
	if (optmode.eq.0) then
		write(202,102)year(d),month(d),day(d),d,h,rainvec(d),&
			tmax(d),tmin(d),parvec(d),vd_d/24.,esoil_d,&
...
!
	reff=0.
	changef= (0.95*mqx - mqssmin)/(0.05*mqx)								! (3.47)
	reff(1:pos) = 0.5*ruptkvec_d(1:pos)/&									! (3.48)
