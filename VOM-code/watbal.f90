...
!!$endif
  ...
  if(nlayersnew.eq.1) then
     sunewvec(1) = (1.d0 / (1.d0 + 0.5d0 ** nvg * (alphavg * yunew) ** nvg)) ** mvg  ! The soil moisture left behind is at field capacity
     pcapnewvec(1) = (-1.d0 + sunewvec(1) ** (-1.d0 / mvg)) ** & ! (Out[54])
          (1.d0 / nvg) / alphavg
     sunewvec(nlayersnew + 1:M) = 1.d0
     pcapvec(nlayersnew + 1:M) = 0.d0
  else
